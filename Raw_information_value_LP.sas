...
/*  %if %sysfunc(exist(autobin_binwise)) %then %do; proc delete data = autobin_&woeout. ; run; %end;*/
  ...
  set &libin..&indata end=last;
			...
			keep  &keeplst. dv weight obs tot_obs tot_dvs _pd_;
  ...
  proc sql  ;
		...
		where 
				...
				libname = 'WORK' 				and	
				type 	= 'char' 				and 
	          	upcase(name) not in ("%upcase(&dv)",'DV',"%upcase(&weight)",'WEIGHT','OBS','TOT_OBS','TOT_DVS',"%upcase(&pd)",'_PD_')
	    order by name ;
  quit ;
  		...
  		%do i = 1 %to &Numvar. ;    
  			%let var = %upcase(%scan(&Num_varlst,&i)) ;
				proc rank data 	= &indata._ group=&group 
						out 	= regout2(keep = obs &var weight dv dv rank_var _pd_);
						var 	&var ;
			        	ranks 	rank_var ;
			    run ;
				...
				proc summary SUM data = raw_test1 nway;
					class rank_var;
					weight weight;
					var  &var. dv _tot_  _pd_;
					output out = raw_test2  (Drop = _FREQ_ _TYPE_ )
										MIN(&var.) 	= Low
										MAX(&var.) 	= High 
										SUM(dv) 	= BandNoofBads
										sum(_tot_) 	= TotalinBand
										mean(_pd_) 	= expected_bad_rate; 
				run;
				
				data raw_test3 ;
					retain rank_var	Var Low	High TotalinBand BandNoofGoods BandNoofBads  	
							TP	GP	BP WoE Info_Val BadRate expected_bad_rate KS Histogram_WOE Histogram_BadRate;
				set  raw_test2 ;
					...
					if GP*BP 			ne 	0 then WoE = log((BandNoofGoods/&Total_Good.)/(BandNoofBads/&Total_Bad.));	else 	
					if BandNoofGoods 	<= .Z or BandNoofGoods = 0 or BandNoofBads <= .Z or BandNoofBads =0 then WoE = .; 
					...
					if woe 				>0 then do;
										if len_woe <0 then 		Histogram_WOE 		= "+";else 								
																Histogram_WOE 		= "+"||repeat("*",len_woe);
					end;
					else do;
										if len_woe < 0 then 	Histogram_WOE 		= "+";else 								
																Histogram_WOE 		= repeat("*",len_woe)||"+";
					...
					if len_BadRate <= 0 then 					Histogram_BadRate 	= 	"+"; else 								
																Histogram_BadRate 	= 	repeat("*",len_BadRate)||"+";
  			...
  			%let var = %upcase(%scan(&char_varlst.,&c.)) ;
				proc sql; create table _char_var1 as 
					select  
						...
						mean(_pd_) 	as							as expected_bad_rate 	format percent8.2
					from &indata._ 
					group by &var.
...
data dev;
set local.vc_scored_dev_dirty;
/*	where ci_samp_flag = 'oot'  ;*/
	wgt=1;
	pd = ranuni(100000);
	keep  cb_ivpr_bad pd wgt &keep_var.;
run;
