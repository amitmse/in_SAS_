/************KS Macro*******************************************************************/;
/* %GINI_CALCULATION, from GINI.sas (amitmse/in_SAS_), byte-identical to the source     */
/* macro definition. The source file's own driver read "cur" (an external scored       */
/* dataset with a bad-flag and a model score) that isn't in the repo, so for this       */
/* Jenner compatibility bundle "cur" is a small mock scored dataset built with the      */
/* same column shape the macro expects (a 0/1 bad flag + a continuous score) -         */
/* everything else, including the macro body and the call below, is unmodified.        */
...
/* Mock scored data standing in for the source driver's "cur" (an external dataset
   not shipped with the repo) - same shape the macro reads: a 0/1 bad flag
   (cb_achl_bad) and a continuous model score (final_score). */
data cur;
	input cb_achl_bad final_score;
	datalines;
	1 410
	1 455
	0 610
